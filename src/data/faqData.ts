...
export const allFAQs: FAQItem[] = [
  ...
  },
  {
    ...
    answer: "We handle dry van, flatbed, reefer, hotshot, machinery, consumer goods, industrial materials, and specialized loads."
  },
  {
    id: 4,
    category: "General",
  ...
  switch (serviceType) {
    case "LTL": // Less Than Truckload
      relevantFAQs = allFAQs.filter(faq => 
        [0, 6, 7, 8, 9, 10, 11, 14, 16, 20].includes(faq.id)
      );
      break;

    case "FTL": // Full Truckload
      relevantFAQs = allFAQs.filter(faq => 
        [0, 6, 8, 9, 10, 11, 12, 14, 16, 20].includes(faq.id)
      );
      break;

    case "Interstate": // Interstate Transport
      relevantFAQs = allFAQs.filter(faq => 
        [2, 6, 8, 10, 11, 12, 16, 18, 19, 20].includes(faq.id)
      );
      break;

    case "Reefer": // Reefer Transport
      relevantFAQs = allFAQs.filter(faq => 
        [3, 6, 8, 10, 11, 13, 16, 17, 19, 20].includes(faq.id)
      );
      break;
    ...
    default:
      // Return general FAQs
      relevantFAQs = allFAQs.filter(faq => 
        [0, 1, 2, 6, 8, 11, 16, 19, 20, 21].includes(faq.id)
      );
  }
